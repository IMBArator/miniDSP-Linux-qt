...
- A feature panel area:
  ...
  - A **Reset** button in the header of every feature panel (Gate / PEQ / Crossover / Compressor) snaps just that feature on the displayed channel — plus any linked slaves — back to the F00 factory defaults after a confirmation dialog. Values are sourced live from the protocol library's `factory_defaults.toml`, so they always match what the firmware considers "blank"
...
243 tests covering the device thread, model, virtual DSP, preset picker, routing matrix, PEQ panel, crossover panel, compressor panel + graph, delay panel + graph, channel-linking dialog, channel-linking sync (master → slave fan-out), and .unt read/write round-trip.
...
minidspqt/                     Main package
  ...
  widgets/                     Custom Qt widgets (CompressorGraph, DelayGraph, FreqResponseGraph, GainKnob, GateGraph, LedIndicator, LevelMeter, ParamKnob, PEQGraph, RoutingMatrix, ToggleButton)
...
| Reset to factory defaults | `minidsp.defaults.load_factory_defaults` | Per-feature **Reset** button in the header of every Gate / PEQ / Crossover / Compressor panel. Confirms via dialog, then snaps only that feature on the displayed channel (plus any linked slaves) back to the F00 factory values; other channels and other features stay untouched. Defaults are sourced live from the protocol library's bundled `factory_defaults.toml` so they always match the firmware |
